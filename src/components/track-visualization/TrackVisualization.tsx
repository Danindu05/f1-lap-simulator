...
import { getTrackOptimization } from "@/lib/utils/trackConfig";
...
export function TrackVisualization({}: TrackVisualizationProps) {
  ...
  // World → Canvas with robust centering after mirroring + arbitrary rotation.
  // Approach: center-based transform so rotated bounding box always fits within padding.
  const worldToCanvas = (x: number, y: number) => {
    ...
    // Orientation flags & rotation (deg → rad)
    ...
    // Compute oriented bounding box extents after rotation for scaling.
    // After mirroring, extents unchanged in magnitude, so we can ignore mirror in size calc.
    ...
    // Apply mirroring about center
    if (mirrorX) rx = -rx;
    if (mirrorY) ry = -ry;

    // Rotate about origin (which is track center now)
    ...
    // Apply content offsets (pre camera) if provided
  ...
  // Initialize camera from track config when session/location changes
  useEffect(() => {
    ...
    trackOpts.initialZoom,
    trackOpts.initialPanX,
    trackOpts.initialPanY,
      ...
      >
        <CardContent className="p-0 w-full h-full">
          <div className="text-center">
            ...
            <h3 className="text-lg font-semibold mb-2 text-muted-foreground">Ready to Analyze</h3>
      ...
      >
        <CardContent className="p-0 w-full h-full">
          <div className="text-center">
            ...
            <h3 className="text-lg font-semibold mb-2 text-muted-foreground">Select Drivers</h3>
            <p className="text-muted-foreground">
